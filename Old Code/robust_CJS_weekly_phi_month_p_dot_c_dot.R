...
sink("robust_CJS_weekly_phi_month_p_dot_c_dot.bug")
cat("
    model{
    ...
    # how do I set a prior to 0? (I don't want to estimate a param for jan if I have an intercept term)

    for(u in 1:12){
      alpha.month[u] ~ dnorm(0, 0.4)T(-10,10)     # priors for monthly survival
    }
    ...
    for(i in 1:nind){
      for(w in 1:(n.weeks-1)){  
        ...
        logit(phi[i,w]) <-  alpha.month[covariate.month[w]] # + alpha.0 + alpha.NDVI * NDVI[i,w] 
