...
################################################################################
IDs <- sort (unique(Zuni.pema.data$tag))
Zuni.secondary.occasions <- unlist(lapply(Zuni.Session.days,function(x){length(x)}))
Ch.array <- array(NA,dim=c(length(IDs),length(Zuni.Session.days),max(Zuni.secondary.occasions)))

for(i in 1:length(IDs)){
  for(m in 1:length(Zuni.Session.days)){
    days <- Zuni.Session.days[[m]]
    for(d in 1:length(days)){
      Ch.array[i,m,d] <- ifelse(length(which(Zuni.pema.data$tag==IDs[i] & Zuni.pema.data$date==days[d]))>0,1,0)
    }
  }
  dimnames(Ch.array) <- list(IDs,NULL,NULL)
  cat("indiv = ", i, "/771", "\n")
}

Zuni.pema.Ch.secondary.array <- Ch.array
...
Zuni.monthly.temporal.covariates <- monthly.temporaldata.fun(capture.data=UNMcaptures, site="Zuni",web=c(1,2,3))
...
save(Zuni.pema.Ch.secondary,Zuni.Session.days,Zuni.weekly.temporal.covariates,Zuni.primary.time.int.weeks,Zuni.individual.covariates, Zuni.secondary.occasions, Zuni.pema.Ch.secondary.array, file="ZunipemaCH.RData")
