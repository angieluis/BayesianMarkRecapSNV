...
cat("
    ...
    for (t in 1:(n.primary.occasions-1)){
      ...
      psiSI[i,t] <- mean.beta*I.dat[t]  # think about indexing here 
        # really should calculate I here based on p (num.caught/p). but I think there will be estimation problems, so for now, I'll go with using MNI

      # I don't want t a logit transform on beta/psi because of the S shape
      # max MNI at C11 is 12, so need to make sure beta is bounded so that beta*12<1
      # This also doens't account for infected immigration.  
    ...
    } ## might want to change this to include recap (c)
    ...
    for(t in 1:n.primary.occasions){
      alpha.t[t] ~ dnorm(0, 0.4)T(-10,10)
    }
    
    mean.beta ~ dunif(0, 1/max(I.dat))  # uniform - where the highest beta could be probability of becoming infected=1 at highest MNI
