...
# 4) line up with latest code for multi-sites and months not trapped (and Emily's data)
...
# need to create 2 columns - caught and infected
# caught = 1 if caught, 0 if not caughts
# infected = 1 if uninfected, and 0 if infected
...
bugs.data <- list(
  ...
  y_caught = obs.dat$caught, # caught
  y_infected = obs.dat$infected, # caught and observed as infected (NA if infection status unknown)
