...
# Model assuming phi with a dummy time-varying covariate (repeated observed seasonality) 


#################################specify model in BUGS language
sink("robust_CJS_phi_dummyx_p_dot_c_dot.bug")
cat("
    model{
    
    ###############Priors and constraints
    
      alpha0 ~ dnorm(0, 0.4)T(-10,10)     #prior for params on temporal covariate for phi 
      alpha1 ~ dnorm(0, 0.4)T(-10,10)
    ...
    for(i in 1:nind){
      for(m in 1:(n.primary.occasions-1)){  
        ...
        logit(phi[i,m]) <- alpha0 + alpha1*dummy.phix[m]   
