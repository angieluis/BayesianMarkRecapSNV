...
monthly.covariate.function <-function(
  ...
  month.data$season <- ifelse(month.data$month==12|month.data$month==1|month.data$month==2|month.data$month==3, 1, ifelse(month.data$month==4|month.data$month==5|month.data$month==6,2,ifelse(month.data$month==7|month.data$month==8|month.data$month==9,3,4)))
  ...
  if(length(temporal.data)>0){ # |length(diversity.data)>0
      ...
      ind.na <- ind.na[-which(ind.na[,2]<9),] # these columns have NAs that need to be filled for CJS models to run. They are likely the first time points when there is a lag. So fill in the next value.
...
multisite.monthly.covariate.function <-function(
  ...
  if(length(temporal.data)>0){ # |length(diversity.data)>0
    ...
    ind.na <- which(is.na(month.data),arr.ind = TRUE)
    ind.na <- ind.na[-which(ind.na[,2]<9),] # these columns have NAs that need to be filled for CJS models to run. They are likely the first time points when there is a lag. So fill in the next value. (this is after removing the first 7 column because that is just the session numbers etc. not the covariate data)
