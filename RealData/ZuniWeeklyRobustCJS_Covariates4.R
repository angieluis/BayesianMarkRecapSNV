...
date()
obs.dat.full <- weekly.longdataCH.fun(CH.secondary, temporal.covariates$weekly.longdata, p_or_c=FALSE)
date() 

p.or.c <- p.or.c.array.fun(CH.secondary, temporal.covariates)
...
bugs.data <- list(
  ...
  weeks.trapped = temporal.covariates$weekly.longdata$week[which(is.finite(temporal.covariates$weekly.longdata$Prim))],
  #time.int = time.int,
  ...
  covariate.prim = temporal.covariates$weekly.longdata$Prim,
...
date() # 23 hours (changed the p or c usage since this one)
