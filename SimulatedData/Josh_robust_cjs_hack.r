...
model{
  ...
  for(i in 1:n.obs){   
    ...
    y[i] ~ dbern(mu1[id[i],prim[i]]) # this isn't right because referring to survival instaed of recapture rate, and need to make it p or c
...
#  Now create a long tibble (i.e. data.frame) from the observation data
# for(i in 1:length(sim.data$observed.month.list)){
  # data.frame(
    # sim.data$observed.month.list[[i]],
    # ID = 1:nrow(sim.data$observed.month.list[[i]])
  # ) %>%
...
system.time(
  robust.cjs <- jags(
    ...
    inits,
    parameters,
    "robust_cjs_raggedarray.bug",
    n.chains = nc,
    n.thin = nt,
    n.iter = ni,
    n.burnin = nb
  )
) #tell how long it ran
